// Demo test-case sets per "company" dataset. Each set shares the exact same
// data structure (the schema/contract), which is what makes the datasets
// plug-and-play and switchable in the UI.

const amazon = [
  // ── Cart ──────────────────────────────────────────────────────────────────
  {
    title: 'Add a single item to the cart from the product page',
    area: 'Cart',
    category: 'Add to Cart',
    testNature: 'Positive',
    type: 'Automated',
    preconditions: 'User is signed in. Product "Echo Dot (5th Gen)" is in stock.',
    testData: 'Product: Echo Dot (5th Gen), ASIN B09B8V1LZ3',
    testSteps:
      '1. Open the product detail page.\n2. Click "Add to Cart".\n3. Open the cart.',
    expectedResult:
      'Cart count increments to 1 and the item appears in the cart with the correct price.',
    comments: 'Core happy path — covered by automation in the smoke suite.',
    pinned: true,
  },
  {
    title: 'Update item quantity in the cart',
    area: 'Cart',
    category: 'Quantity',
    testNature: 'Positive',
    type: 'Manual',
    preconditions: 'Cart contains at least one item.',
    testData: 'Quantity change: 1 -> 3',
    testSteps:
      '1. Open the cart.\n2. Change the quantity dropdown to 3.\n3. Observe the subtotal.',
    expectedResult: 'Quantity updates to 3 and the subtotal recalculates correctly.',
  },
  {
    title: 'Remove an item from the cart',
    area: 'Cart',
    category: 'Add to Cart',
    testNature: 'Positive',
    type: 'Automated',
    preconditions: 'Cart contains exactly one item.',
    testSteps: '1. Open the cart.\n2. Click "Delete" under the item.',
    expectedResult: 'Item is removed and the cart shows the empty-cart state.',
  },
  {
    title: 'Save an item for later',
    area: 'Cart',
    category: 'Saved for Later',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Open the cart with one item.\n2. Click "Save for later".',
    expectedResult:
      'Item moves to the "Saved for later" section and is removed from the active cart.',
  },
  {
    title: 'Cart persists across sessions for a signed-in user',
    area: 'Cart',
    category: 'Persistence',
    testNature: 'Positive',
    type: 'Manual',
    preconditions: 'Signed-in user adds an item, then signs out and back in.',
    testSteps:
      '1. Add an item to the cart.\n2. Sign out.\n3. Sign back in.\n4. Open the cart.',
    expectedResult: 'Previously added item is still present in the cart.',
    comments: 'FAILS intermittently on staging — cart cleared after re-login. Bug AMZ-4521.',
    pinned: true,
  },
  {
    title: 'Cart shows correct subtotal with mixed quantities',
    area: 'Cart',
    category: 'Totals',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'Item A x2 @ $19.99, Item B x1 @ $5.00',
    expectedResult: 'Subtotal equals $44.98.',
  },
  {
    title: 'Empty cart displays recommended products',
    area: 'Cart',
    category: 'Recommendations',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Empty cart page shows a "Recommended for you" carousel.',
    comments: 'Deferred — recommendations service not yet wired in test env.',
  },

  // ── Checkout ────────────────────────────────────────────────────────────────
  {
    title: 'Complete checkout with a saved credit card',
    area: 'Checkout',
    category: 'Payment',
    testNature: 'Positive',
    type: 'Automated',
    preconditions: 'User has a default address and a saved Visa card.',
    testData: 'Card: Visa ****4242, Address: Seattle default',
    testSteps:
      '1. Add an item to the cart.\n2. Proceed to checkout.\n3. Confirm address and payment.\n4. Place the order.',
    expectedResult:
      'Order is placed, an order number is shown, and a confirmation email is queued.',
    comments: 'Critical revenue path — runs in every release pipeline.',
    pinned: true,
  },
  {
    title: 'Apply a valid promo code at checkout',
    area: 'Checkout',
    category: 'Discount',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Promo code: SAVE10 (10% off)',
    testSteps: '1. Go to checkout.\n2. Enter promo code SAVE10.\n3. Click "Apply".',
    expectedResult: 'Order total is reduced by 10% and the discount line is shown.',
  },
  {
    title: 'Reject an expired promo code at checkout',
    area: 'Checkout',
    category: 'Discount',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Promo code: EXPIRED2023',
    expectedResult: 'An inline error "This code has expired" is shown; total is unchanged.',
  },
  {
    title: 'Select a different shipping speed',
    area: 'Checkout',
    category: 'Shipping',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. At checkout, open shipping options.\n2. Select "One-Day Shipping".',
    expectedResult: 'Shipping cost and estimated delivery date update accordingly.',
  },
  {
    title: 'Checkout blocked when no shipping address is set',
    area: 'Checkout',
    category: 'Address',
    testNature: 'Negative',
    type: 'Manual',
    preconditions: 'New account with no saved address.',
    expectedResult:
      'User is prompted to add an address before the "Place order" button is enabled.',
    comments: 'Blocked — address microservice returns 503 in test env. Ticket OPS-220.',
    pinned: false,
  },
  {
    title: 'Place an order using a gift card balance',
    area: 'Checkout',
    category: 'Gift Card',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Gift card balance: $25.00',
    expectedResult:
      'Gift card balance is applied first and any remainder charges the default card.',
  },
  {
    title: 'Order confirmation page shows correct itemized totals',
    area: 'Checkout',
    category: 'Order Summary',
    testNature: 'Positive',
    type: 'Automated',
    expectedResult:
      'Confirmation page lists items, subtotal, shipping, tax, and grand total matching the cart.',
  },

  // ── Search ──────────────────────────────────────────────────────────────────
  {
    title: 'Search returns relevant results for a product keyword',
    area: 'Search',
    category: 'Relevance',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'Query: "wireless headphones"',
    testSteps: '1. Type "wireless headphones" in the search bar.\n2. Press Enter.',
    expectedResult: 'Results page shows headphone products with the keyword highlighted.',
  },
  {
    title: 'Search autocomplete suggests popular queries',
    area: 'Search',
    category: 'Autocomplete',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Partial query: "echo"',
    expectedResult: 'Dropdown suggests "echo dot", "echo show", etc. within ~300ms.',
  },
  {
    title: 'Filter search results by price range',
    area: 'Search',
    category: 'Filtering',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Search "laptop".\n2. Set price filter $500-$1000.\n3. Apply.',
    expectedResult: 'Only products within the $500-$1000 range are shown.',
  },
  {
    title: 'Sort search results by customer rating',
    area: 'Search',
    category: 'Sorting',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Results re-order with the highest-rated products first.',
  },
  {
    title: 'Search with no results shows a helpful empty state',
    area: 'Search',
    category: 'Empty State',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Query: "asdkjhqwezxc"',
    expectedResult: 'A "No results found" message with spelling suggestions is shown.',
    comments: 'FAILS — currently shows a blank page instead of the empty state. Bug AMZ-4610.',
    pinned: false,
  },
  {
    title: 'Search respects department/category scope',
    area: 'Search',
    category: 'Scope',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Select "Books" in the category dropdown.\n2. Search "python".',
    expectedResult: 'Only book results are returned, not electronics.',
  },
  {
    title: 'Voice search returns results (Alexa app)',
    area: 'Search',
    category: 'Voice',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Spoken query is transcribed and returns matching results.',
    comments: 'Deferred to next sprint — voice harness not ready.',
  },

  // ── Account ─────────────────────────────────────────────────────────────────
  {
    title: 'Sign in with valid credentials',
    area: 'Account',
    category: 'Sign In',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'Test account: shopper01@example.com',
    testSteps: '1. Open sign-in.\n2. Enter email and password.\n3. Submit.',
    expectedResult: 'User is signed in and redirected to the home page.',
    pinned: false,
  },
  {
    title: 'Sign in fails with an incorrect password',
    area: 'Account',
    category: 'Sign In',
    testNature: 'Negative',
    type: 'Automated',
    expectedResult: 'An error "Your password is incorrect" is shown; user stays signed out.',
  },
  {
    title: 'Update the default shipping address',
    area: 'Account',
    category: 'Address',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Go to Account > Addresses.\n2. Edit the default address.\n3. Save.',
    expectedResult: 'New address is saved and shown as default at checkout.',
  },
  {
    title: 'Change account password',
    area: 'Account',
    category: 'Password',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Password is updated and the user can sign in with the new password.',
  },
  {
    title: 'View order history',
    area: 'Account',
    category: 'Orders',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Past orders are listed newest-first with status and total.',
  },
  {
    title: 'Enable two-step verification',
    area: 'Account',
    category: 'Security',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'After enabling 2SV, sign-in prompts for an OTP code.',
  },
  {
    title: 'Account lockout after repeated failed sign-ins',
    area: 'Account',
    category: 'Security',
    testNature: 'Negative',
    type: 'Manual',
    testSteps: '1. Enter a wrong password 6 times.',
    expectedResult: 'Account is temporarily locked and a recovery message is shown.',
    comments: 'Blocked — rate-limiter disabled in test env. Need OPS to enable.',
  },
  {
    title: 'Manage Prime membership settings',
    area: 'Account',
    category: 'Membership',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'User can view renewal date and toggle auto-renew.',
  },

  // ── Payments ────────────────────────────────────────────────────────────────
  {
    title: 'Add a new credit card to the wallet',
    area: 'Payments',
    category: 'Wallet',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Card: Mastercard 5555 5555 5555 4444',
    testSteps:
      '1. Go to Account > Payments.\n2. Click "Add a card".\n3. Enter card details and save.',
    expectedResult: 'Card is validated, masked, and listed in the wallet.',
  },
  {
    title: 'Decline an invalid card number',
    area: 'Payments',
    category: 'Card Validation',
    testNature: 'Negative',
    type: 'Automated',
    testData: 'Card: 1234 5678 9012 3456 (fails Luhn check)',
    expectedResult: 'An inline error "Enter a valid card number" is shown.',
  },
  {
    title: 'Set a default payment method',
    area: 'Payments',
    category: 'Default Method',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Selected card is marked default and pre-selected at checkout.',
  },
  {
    title: 'Remove a saved payment method',
    area: 'Payments',
    category: 'Wallet',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Card is removed from the wallet after confirmation.',
  },
  {
    title: 'Payment fails gracefully on bank decline',
    area: 'Payments',
    category: 'Decline Handling',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Test card simulating issuer decline.',
    testSteps: '1. Place an order with the decline test card.',
    expectedResult:
      'A clear "Your payment was declined" message is shown and the order is not created.',
    comments: 'FAILS — order is created in a pending state despite the decline. Bug AMZ-4700. URGENT.',
    pinned: true,
  },
  {
    title: 'Process a refund to the original payment method',
    area: 'Payments',
    category: 'Refunds',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Refund is issued to the original card and reflected in order history.',
  },
  {
    title: 'Pay using Amazon gift card balance only',
    area: 'Payments',
    category: 'Gift Card',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Gift card balance: $60.00; order total $42.50',
    expectedResult: 'Order is fully paid from the gift card and balance drops to $17.50.',
  },
  {
    title: 'Currency is displayed correctly for the locale',
    area: 'Payments',
    category: 'Localization',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Prices show the correct currency symbol and formatting per locale.',
  },

  // ── Additional negative test cases (error / invalid-input handling) ──────────
  {
    title: 'Adding an out-of-stock item to the cart is rejected',
    area: 'Cart',
    category: 'Add to Cart',
    testNature: 'Negative',
    type: 'Manual',
    preconditions: 'Product is marked out of stock.',
    testSteps:
      '1. Open an out-of-stock product page.\n2. Attempt to click "Add to Cart".',
    expectedResult:
      'The "Add to Cart" button is disabled and a "Currently unavailable" notice is shown; the cart count does not change.',
  },
  {
    title: 'Setting cart quantity to zero or a negative number is blocked',
    area: 'Cart',
    category: 'Quantity',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Quantity input: 0, then -1',
    testSteps:
      '1. Open the cart.\n2. Type 0 into the quantity field.\n3. Type -1 into the quantity field.',
    expectedResult:
      'Invalid quantities are rejected with a validation message; quantity reverts to the last valid value.',
    comments: 'FAILS — entering 0 currently removes the item without confirmation. Bug AMZ-4815.',
  },
  {
    title: 'Checkout rejects an invalid promo code',
    area: 'Checkout',
    category: 'Discount',
    testNature: 'Negative',
    type: 'Automated',
    testData: 'Promo code: NOTAREALCODE',
    testSteps: '1. At checkout, enter NOTAREALCODE.\n2. Click "Apply".',
    expectedResult: 'An inline error "Invalid code" is shown and the order total is unchanged.',
  },
  {
    title: 'Checkout rejects an incomplete shipping address',
    area: 'Checkout',
    category: 'Address',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Address missing ZIP/postal code.',
    testSteps:
      '1. Enter an address with no postal code.\n2. Try to continue to payment.',
    expectedResult:
      'A field-level error highlights the missing postal code and prevents continuing.',
  },
  {
    title: 'Search input rejects an overly long query string',
    area: 'Search',
    category: 'Relevance',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Query: a 5,000-character string',
    expectedResult:
      'The query is truncated to the max length or rejected gracefully; no server error (500) occurs.',
  },
  {
    title: 'Sign up is rejected for a malformed email address',
    area: 'Account',
    category: 'Sign In',
    testNature: 'Negative',
    type: 'Automated',
    testData: 'Email: "not-an-email"',
    testSteps: '1. Open create account.\n2. Enter "not-an-email".\n3. Submit.',
    expectedResult: 'An inline error "Enter a valid email" is shown; the account is not created.',
  },
  {
    title: 'Adding an expired credit card is rejected',
    area: 'Payments',
    category: 'Card Validation',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Card with expiry 01/2020.',
    testSteps: '1. Go to Payments > Add a card.\n2. Enter a card expiring 01/2020.\n3. Save.',
    expectedResult: 'An error "This card has expired" is shown and the card is not saved.',
  },
  {
    title: 'Refund greater than the original charge is rejected',
    area: 'Payments',
    category: 'Refunds',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Original charge $42.50; attempted refund $50.00.',
    expectedResult: 'The over-refund is blocked with a clear error; no money is moved.',
    comments: 'Blocked — refund sandbox returns 500 for over-amount in test env. Ticket OPS-241.',
  },

  // ── API tests (layer = API; use endpoint/method/request/status fields) ───────
  {
    title: 'POST /cart/items adds an item',
    area: 'Cart',
    category: 'Add to Cart',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/cart/items',
    httpMethod: 'POST',
    requestBody: '{ "asin": "B09B8V1LZ3", "qty": 1 }',
    expectedStatus: '201',
    preconditions: 'Valid auth token for a signed-in customer.',
    expectedResult: 'Response contains the created line item and updated cart subtotal.',
    pinned: true,
  },
  {
    title: 'GET /cart returns the current cart',
    area: 'Cart',
    category: 'Totals',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/cart',
    httpMethod: 'GET',
    expectedStatus: '200',
    expectedResult: 'Returns items, subtotal, and currency for the authenticated user.',
  },
  {
    title: 'POST /checkout with an invalid card is rejected',
    area: 'Checkout',
    category: 'Payment',
    testNature: 'Negative',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/checkout',
    httpMethod: 'POST',
    requestBody: '{ "card": "1234567890123456", "amount": 4250 }',
    expectedStatus: '422',
    expectedResult: 'Returns a 422 with an error code CARD_INVALID; no order is created.',
  },
  {
    title: 'POST /checkout/promo applies a valid promo code',
    area: 'Checkout',
    category: 'Discount',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/checkout/promo',
    httpMethod: 'POST',
    requestBody: '{ "code": "SAVE10" }',
    expectedStatus: '200',
    expectedResult: 'Returns the recalculated total with a 10% discount line.',
  },
  {
    title: 'GET /search returns results for a keyword',
    area: 'Search',
    category: 'Relevance',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/search?q=wireless+headphones',
    httpMethod: 'GET',
    expectedStatus: '200',
    expectedResult: 'Returns a paginated list of products with relevance scores.',
  },
  {
    title: 'POST /refunds over the original amount is rejected',
    area: 'Payments',
    category: 'Refunds',
    testNature: 'Negative',
    type: 'Automated',
    layer: 'API',
    endpoint: '/api/refunds',
    httpMethod: 'POST',
    requestBody: '{ "orderId": "111-222", "amount": 5000 }',
    expectedStatus: '409',
    expectedResult: 'Returns 409 REFUND_EXCEEDS_CHARGE; balance is unchanged.',
  },
];

// ── Google dataset — same structure, Google-product test cases ────────────────
const google = [
  // ── Search ──────────────────────────────────────────────────────────────────
  {
    title: 'Search returns relevant web results for a query',
    area: 'Search',
    category: 'Relevance',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'Query: "weather today"',
    testSteps: '1. Open google.com.\n2. Type "weather today".\n3. Press Enter.',
    expectedResult: 'A relevant results page with a weather panel at the top is shown.',
    comments: 'Core path — runs in the search smoke suite.',
    pinned: true,
  },
  {
    title: 'Autocomplete suggests queries as you type',
    area: 'Search',
    category: 'Autocomplete',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Partial query: "how to"',
    expectedResult: 'A suggestion dropdown appears within ~200ms with popular completions.',
  },
  {
    title: 'Knowledge panel appears for a well-known entity',
    area: 'Search',
    category: 'Knowledge Panel',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Query: "Albert Einstein"',
    expectedResult: 'A knowledge panel with summary, dates, and image is shown on the right.',
  },
  {
    title: 'Safe-search filters explicit results',
    area: 'Search',
    category: 'Safe Search',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'With SafeSearch on, explicit results are filtered out of the results page.',
  },
  {
    title: 'Misspelled query shows "Did you mean" suggestion',
    area: 'Search',
    category: 'Spelling',
    testNature: 'Negative',
    type: 'Automated',
    testData: 'Query: "recieve email"',
    expectedResult: 'A "Did you mean: receive email" correction is offered.',
  },
  {
    title: 'Empty query submission is handled gracefully',
    area: 'Search',
    category: 'Validation',
    testNature: 'Negative',
    type: 'Manual',
    testSteps: '1. Focus the search box.\n2. Press Enter with no text.',
    expectedResult: 'The page stays on the homepage; no error is shown.',
  },

  // ── Gmail ─────────────────────────────────────────────────────────────────--
  {
    title: 'Compose and send an email',
    area: 'Gmail',
    category: 'Compose',
    testNature: 'Positive',
    type: 'Automated',
    preconditions: 'User is signed in to Gmail.',
    testData: 'To: teammate@example.com, Subject: "Hello"',
    testSteps:
      '1. Click Compose.\n2. Fill recipient, subject, body.\n3. Click Send.',
    expectedResult: 'Email is sent and appears in the Sent folder.',
    pinned: true,
  },
  {
    title: 'Save a draft automatically while composing',
    area: 'Gmail',
    category: 'Drafts',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Start composing.\n2. Type a few lines.\n3. Close the compose window.',
    expectedResult: 'A draft is saved automatically and is visible in Drafts.',
  },
  {
    title: 'Attach a file to an email',
    area: 'Gmail',
    category: 'Attachments',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Attachment: report.pdf (2 MB)',
    testSteps: '1. Compose.\n2. Click the attach icon.\n3. Select report.pdf.\n4. Send.',
    expectedResult: 'The attachment uploads and is delivered with the email.',
    comments: 'FAILS — upload stalls for files >1 MB on staging. Bug GML-3312.',
  },
  {
    title: 'Reject sending an email to a malformed address',
    area: 'Gmail',
    category: 'Validation',
    testNature: 'Negative',
    type: 'Automated',
    testData: 'To: "not-an-email"',
    expectedResult: 'A validation error is shown and the email is not sent.',
  },
  {
    title: 'Spam message is routed to the Spam folder',
    area: 'Gmail',
    category: 'Spam',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'A known-spam test message lands in Spam, not the Inbox.',
  },
  {
    title: 'Search within mailbox returns matching emails',
    area: 'Gmail',
    category: 'Search',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Query: from:teammate has:attachment',
    expectedResult: 'Only emails from the teammate with attachments are listed.',
  },
  {
    title: 'Attaching an oversized file is blocked with a clear message',
    area: 'Gmail',
    category: 'Attachments',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'Attachment: 30 MB file (limit 25 MB)',
    expectedResult: 'A "File exceeds 25 MB" message is shown and a Drive-link option is offered.',
  },

  // ── Drive ─────────────────────────────────────────────────────────────────--
  {
    title: 'Upload a file to Drive',
    area: 'Drive',
    category: 'Upload',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'File: notes.docx',
    testSteps: '1. Open Drive.\n2. Click New > File upload.\n3. Choose notes.docx.',
    expectedResult: 'File uploads and appears in My Drive with the correct name and size.',
  },
  {
    title: 'Share a file with view-only permission',
    area: 'Drive',
    category: 'Sharing',
    testNature: 'Positive',
    type: 'Manual',
    testSteps:
      '1. Right-click a file > Share.\n2. Add a person as Viewer.\n3. Send.',
    expectedResult: 'The recipient can open and view but cannot edit the file.',
    pinned: true,
  },
  {
    title: 'Create a shared folder and add files',
    area: 'Drive',
    category: 'Folders',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'A new folder is created and files moved into it inherit sharing.',
  },
  {
    title: 'Restore a file from Trash',
    area: 'Drive',
    category: 'Trash',
    testNature: 'Positive',
    type: 'Manual',
    testSteps: '1. Delete a file.\n2. Open Trash.\n3. Restore it.',
    expectedResult: 'The file returns to its original location.',
  },
  {
    title: 'Uploading an unsupported file type is rejected',
    area: 'Drive',
    category: 'Upload',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'File: malware.exe (blocked type)',
    expectedResult: 'Upload is blocked with a clear "file type not allowed" message.',
  },
  {
    title: 'Revoking access removes the file from the recipient',
    area: 'Drive',
    category: 'Sharing',
    testNature: 'Negative',
    type: 'Manual',
    expectedResult: 'After revoking, the recipient can no longer open the file.',
    comments: 'Blocked — sharing service returns 503 in test env. Ticket OPS-330.',
  },

  // ── Maps ──────────────────────────────────────────────────────────────────--
  {
    title: 'Get driving directions between two places',
    area: 'Maps',
    category: 'Directions',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'From: Mountain View, To: San Francisco',
    testSteps: '1. Open Maps.\n2. Enter origin and destination.\n3. Click Directions.',
    expectedResult: 'A route with distance and ETA is shown on the map.',
    pinned: true,
  },
  {
    title: 'Search for a nearby business category',
    area: 'Maps',
    category: 'Places',
    testNature: 'Positive',
    type: 'Manual',
    testData: 'Query: "coffee near me"',
    expectedResult: 'Nearby coffee shops are pinned with ratings and hours.',
  },
  {
    title: 'Switch to public-transit directions',
    area: 'Maps',
    category: 'Directions',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'Transit options with departure times and line numbers are shown.',
  },
  {
    title: 'Directions for an unreachable route are handled gracefully',
    area: 'Maps',
    category: 'Directions',
    testNature: 'Negative',
    type: 'Manual',
    testData: 'From: an island with no ferry, To: mainland',
    expectedResult: 'A "could not calculate directions" message is shown, not a blank map.',
    comments: 'FAILS — currently spins indefinitely. Bug MAP-2120.',
  },
  {
    title: 'Save a place to a personal list',
    area: 'Maps',
    category: 'Saved Places',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'The place is added to the "Want to go" list and shows a marker.',
  },

  // ── Account ─────────────────────────────────────────────────────────────────
  {
    title: 'Sign in to a Google Account with valid credentials',
    area: 'Account',
    category: 'Sign In',
    testNature: 'Positive',
    type: 'Automated',
    testData: 'Test account: user01@example.com',
    expectedResult: 'User is signed in and lands on the account dashboard.',
  },
  {
    title: 'Two-factor prompt appears on sign-in from a new device',
    area: 'Account',
    category: 'Security',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'A 2-Step Verification prompt is shown before access is granted.',
  },
  {
    title: 'Sign-in is rejected with a wrong password',
    area: 'Account',
    category: 'Sign In',
    testNature: 'Negative',
    type: 'Automated',
    expectedResult: 'An error is shown and access is denied.',
  },
  {
    title: 'Recover account via recovery email',
    area: 'Account',
    category: 'Recovery',
    testNature: 'Positive',
    type: 'Manual',
    expectedResult: 'A recovery code is sent and lets the user reset the password.',
  },
  {
    title: 'Account is locked after repeated failed attempts',
    area: 'Account',
    category: 'Security',
    testNature: 'Negative',
    type: 'Manual',
    expectedResult: 'The account is temporarily locked and a recovery path is offered.',
    comments: 'Blocked — rate limiter disabled in test env.',
  },

  // ── API tests (layer = API) ──────────────────────────────────────────────────
  {
    title: 'GET /customsearch returns results',
    area: 'Search',
    category: 'Relevance',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/customsearch/v1?q=weather',
    httpMethod: 'GET',
    expectedStatus: '200',
    preconditions: 'Valid API key and search engine id.',
    expectedResult: 'Returns a JSON list of search results with titles and links.',
    pinned: true,
  },
  {
    title: 'GET /customsearch with a missing API key is rejected',
    area: 'Search',
    category: 'Validation',
    testNature: 'Negative',
    type: 'Automated',
    layer: 'API',
    endpoint: '/customsearch/v1?q=weather',
    httpMethod: 'GET',
    expectedStatus: '403',
    expectedResult: 'Returns 403 with a "keyInvalid" error reason.',
  },
  {
    title: 'POST /gmail/messages/send sends an email',
    area: 'Gmail',
    category: 'Compose',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/gmail/v1/users/me/messages/send',
    httpMethod: 'POST',
    requestBody: '{ "raw": "<base64-encoded MIME message>" }',
    expectedStatus: '200',
    expectedResult: 'Returns the sent message id and thread id.',
  },
  {
    title: 'POST /drive/files uploads a file',
    area: 'Drive',
    category: 'Upload',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/upload/drive/v3/files?uploadType=media',
    httpMethod: 'POST',
    requestBody: '<binary file content>',
    expectedStatus: '200',
    expectedResult: 'Returns the new file metadata including its id.',
  },
  {
    title: 'GET /directions returns a route',
    area: 'Maps',
    category: 'Directions',
    testNature: 'Positive',
    type: 'Automated',
    layer: 'API',
    endpoint: '/maps/api/directions/json?origin=MountainView&destination=SF',
    httpMethod: 'GET',
    expectedStatus: '200',
    expectedResult: 'Returns routes with legs, distance, and duration.',
  },
  {
    title: 'GET /directions for an impossible route returns ZERO_RESULTS',
    area: 'Maps',
    category: 'Directions',
    testNature: 'Negative',
    type: 'Automated',
    layer: 'API',
    endpoint: '/maps/api/directions/json?origin=island&destination=mainland',
    httpMethod: 'GET',
    expectedStatus: '200',
    expectedResult: 'Returns HTTP 200 with status "ZERO_RESULTS" and an empty routes array.',
  },
];

export const CASE_SETS = {
  Amazon: amazon,
  Google: google,
};
